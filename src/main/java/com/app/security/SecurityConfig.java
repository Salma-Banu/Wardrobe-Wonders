...
//		.and()
		...
		.antMatchers(HttpMethod.OPTIONS).permitAll()//to permit all clients means react also
		.antMatchers("/products/purchase/**").hasRole("CUSTOMER")//only we need to change this on our project
		.antMatchers("/products/add").hasRole("ADMIN")//only we need to change this on our project
		...
		.addFilterBefore(jwtFilter, UsernamePasswordAuthenticationFilter.class);//our custom filter placed before any authentication filter
